        ...
        try {
          ...
          if (normalized && ['A', 'B', 'C', 'D'].includes(normalized) && !excludedAnswers.includes(normalized)) {
            ...
            if (submitButton) {
              ...
              if (nextButton) {
                // Click "Next" or re-click the same Submit to load next question
                nextButton.click();
                await new Promise(r => setTimeout(r, 800));

                // Wait up to 5 seconds for question text to change
                const oldQuestionText = (document.querySelector('#question-text')?.textContent || '').trim();
                let newQuestionText = oldQuestionText;
                for (let i = 0; i < 10; i++) { // 10 tries x 500ms = 5s
                  await new Promise(r => setTimeout(r, 500));
                  newQuestionText = (document.querySelector('#question-text')?.textContent || '').trim();
                  if (newQuestionText && newQuestionText !== oldQuestionText) break;
                }

                const newRadios = document.querySelectorAll('[role="radio"]');
                if (newRadios.length && newQuestionText !== oldQuestionText) {
                  // Question changed → continue loop
                  if (!this.isRunning) return false;
                  return true;
                } else {
                  if (answerContentEl)
                    answerContentEl.textContent = 'No new question detected — stopping.';
                  return false;
                }
              }
