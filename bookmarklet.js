    ...
    class AssessmentHelper {
        constructor() {
            ...
            this.settingsKeys = {
                ...
                cloudflare_data: 'ah_cloudflare_data',
                cloudflare_other: 'ah_cloudflare_other'
        ...
        async fetchAnswer(queryContent, retryCount = 0) {
            ...
            try {
                ...
                if (useDirectApi) {
                    ...
                    let cfAskUrl = ''
                    try {
                        // prefer any provided loader helper, fall back to localStorage
                        if (typeof this.loadSetting === 'function') {
                            cfAskUrl = this.loadSetting(cfKey, '') || ''
                        } else {
                            cfAskUrl = localStorage.getItem(cfKey) || ''
                        }
                    } catch (e) {
                        cfAskUrl = localStorage.getItem(cfKey) || ''
                    }
                    ...
                    if (!cfAskUrl) {
                        throw new Error('Cloudflare /ask endpoint not configured (localStorage key: ' + cfKey + ')')
                    }

                    response = await fetch(cfAskUrl, {
                        method: 'POST',
                        cache: 'no-cache',
        ...
        openAISettings() {
            ...
            const cfAskInput = this.createEl('input', {
                type: 'text',
                value: localStorage.getItem(this.settingsKeys.cloudflare_ask) || '',
                placeholder: 'https://your-cloudflare-host/ask',
            ...
            cfAskReset.addEventListener('click', () => {
                cfAskInput.value = '';
                this.saveSetting(this.settingsKeys.cloudflare_ask, '');
                localStorage.removeItem(this.settingsKeys.cloudflare_ask);
            });
            cfAskInput.addEventListener('change', () => this.saveSetting(this.settingsKeys.cloudflare_ask, cfAskInput.value || ''));
            cfAskRow.appendChild(cfAskLabel);
            cfAskRow.appendChild(cfAskInput);
            cfAskRow.appendChild(cfAskReset);
            panel.appendChild(cfAskRow);

            // Cloudflare /data
            const cfDataRow = this.createEl('div', { style: 'display:flex;align-items:center;gap:8px;margin-bottom:8px;width:100%;' });
            const cfDataLabel = this.createEl('label', { text: 'Cloudflare /data URL:', style: 'min-width:160px;' });
            const cfDataInput = this.createEl('input', {
                type: 'text',
                value: localStorage.getItem(this.settingsKeys.cloudflare_data) || '',
                placeholder: 'https://your-cloudflare-host/data',
                style: 'flex:1;padding:6px;border:1px solid #ccc;border-radius:4px;'
            });
            const cfDataReset = this.createEl('span', { className: 'ah-reset', text: '↺', title: 'Reset' });
            cfDataReset.addEventListener('click', () => {
                cfDataInput.value = '';
                this.saveSetting(this.settingsKeys.cloudflare_data, '');
                localStorage.removeItem(this.settingsKeys.cloudflare_data);
            });
            cfDataInput.addEventListener('change', () => this.saveSetting(this.settingsKeys.cloudflare_data, cfDataInput.value || ''));
            cfDataRow.appendChild(cfDataLabel);
            cfDataRow.appendChild(cfDataInput);
            cfDataRow.appendChild(cfDataReset);
            panel.appendChild(cfDataRow);

            // Cloudflare other
            const cfOtherRow = this.createEl('div', { style: 'display:flex;align-items:center;gap:8px;margin-bottom:8px;width:100%;' });
            const cfOtherLabel = this.createEl('label', { text: 'Cloudflare other URL:', style: 'min-width:160px;' });
            const cfOtherInput = this.createEl('input', {
                type: 'text',
                value: localStorage.getItem(this.settingsKeys.cloudflare_other) || '',
                placeholder: 'https://your-cloudflare-host/whatever',
                style: 'flex:1;padding:6px;border:1px solid #ccc;border-radius:4px;'
            });
            const cfOtherReset = this.createEl('span', { className: 'ah-reset', text: '↺', title: 'Reset' });
            cfOtherReset.addEventListener('click', () => {
                cfOtherInput.value = '';
                this.saveSetting(this.settingsKeys.cloudflare_other, '');
                localStorage.removeItem(this.settingsKeys.cloudflare_other);
            });
            cfOtherInput.addEventListener('change', () => this.saveSetting(this.settingsKeys.cloudflare_other, cfOtherInput.value || ''));
            cfOtherRow.appendChild(cfOtherLabel);
            cfOtherRow.appendChild(cfOtherInput);
            cfOtherRow.appendChild(cfOtherReset);
            panel.appendChild(cfOtherRow);
            ...
            const attemptOnce = async (excludedAnswers = []) => {
                ...
                try {
                    ...
                    if (tinyIframe || plainTextarea || contentEditable) {
                        ...
                        const answerContainerEl = document.getElementById('answerContainer');
                        const answerContentEl = answerContainerEl ? answerContainerEl.querySelector('#answerContent') : null;
                        if (answerContentEl) answerContentEl.textContent = normalized || (raw.trim().length ? raw.trim() : answer);
                        if (answerContainerEl) { answerContainerEl.style.display = 'flex'; answerContainerEl.style.visibility = 'visible'; answerContainerEl.classList.add('show'); }
